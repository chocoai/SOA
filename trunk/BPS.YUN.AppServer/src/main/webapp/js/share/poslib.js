...
var posLib = {
	...
	getScreenLeft:	function (el) {
		...
		var result = w.screenLeft + this.getBorderLeftWidth(this.getCanvasElement(el)) + this.getClientLeft(el);
	...
	getScreenTop:	function (el) {
		var doc = el.ownerDocument || el.document;	// IE55 bug
		var w = doc.parentWindow;
		var result = w.screenTop + this.getBorderTopWidth(this.getCanvasElement(el)) + this.getClientTop(el);
		if( w.frameElement != null )
			return result - 2;
		else
			return result;
