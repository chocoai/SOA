...
public class SoaMemberSimple implements ISOAService {
    ...
    public JSONObject allowDaikouSimple(JSONObject json) throws Exception {
        ...
        try {
            ...
            if(!isAuthed){
                throw new BizException(ErrorCode.USER_IS_NO_REALNAME, "参数isAuthed错误,未进行实名认证");
            }
            if(!isBinded){
                throw new BizException(ErrorCode.CARD_BIND, "参数isBinded错误,未绑定银行卡");
            }
            if(!Boolean.TRUE.equals(memberEntity.get("isIdentity_checked"))){
                throw new BizException(ErrorCode.USER_IS_NO_REALNAME, "未进行实名认证");
            }
            List<Map<String, Object>> retMap = Extension.memberService.getBindBankCardList(memberId,1L);
            if(retMap.size() <= 0 || retMap == null){
                throw new BizException(ErrorCode.CARD_BIND, "未绑定银行卡");
            }
            logger.info("openDaikouSimple参数：applicationId=" + applicationId + ",bizUserId=" + bizUserId + ",isAuthed=" + isAuthed + ",isBinded=" + isBinded);
            Map<String, Object> resp = Extension.memberService.openDaikouSimple(applicationId, bizUserId, isAuthed, isBinded,memberId);
