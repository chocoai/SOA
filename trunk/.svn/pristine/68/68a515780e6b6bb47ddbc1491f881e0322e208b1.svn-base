...
try{
	...
	String key = (String)Extension.paramMap.get("AlipayConfig.gateway.key"); 		 //通联支付 安全校验码 
	//字符集 默认填1，固定选择值：1、2、3；1代表UTF-8；2代表GBK；3代表GB2312；
	inputCharset= (String)Extension.paramMap.get("AlipayConfig.gateway.inputCharset");
	//前台通知页面
	...
	version = (String)Extension.paramMap.get("AlipayConfig.gateway.version");
	//语言种类 固定值：1；1代表简体中文、2代表繁体中文、3代表英文
	language= (String)Extension.paramMap.get("AlipayConfig.gateway.language");
	//签名类型  默认填1，固定选择值：0、1；0表示客户用MD5验签，1表示客户用证书验签
	signType = (String)Extension.paramMap.get("AlipayConfig.gateway.signType");
	//通联支付 商户号  
	merchantId = (String)Extension.paramMap.get("AlipayConfig.gateway.merchantId");
	payerName = "";
	payerEmail = "";
	payerTelephone = "";
	payerIDCard = "";
	...
	if("28".equals(payType)){
		...
		String certPath = (String)Extension.paramMap.get("AlipayConfig.gateway.TLCertUrl");
	...
	isError = true;
	returnUrl = exceptionOpe(bizId, payCompleteFrontUrl, bizE.getErrorCode(), bizE.getMessage());
}catch(Exception e){
	logger.error(e.getMessage(), e);
	
	isError = true;
	returnUrl = exceptionOpe(bizId, payCompleteFrontUrl, ErrorCode.OTHER_ERROR, e.getMessage());
}
%>
...
//失败操作，返回参数
public String exceptionOpe(String sysid, String payCompleteFrontUrl, String errorCode, String message) throws BizException{
	logger.info("==========================exceptionOpe start==========================");
	logger.info("exceptionOpe参数：sysid=" + sysid + ",,errorCode=" + errorCode + ",,message=" + message);
	...
	try{
		...
		rps.put("status", "error");
		rps.put("errorCode", errorCode);
		
		rps.put("message", message.contains("&") ? "内部错误。" : message);
		
		//签名
		String signStr = sysid + rps.toString() + timestamp;
		String sign = SoaServiceUtil.rsaEncrypt(sysid, signStr);
		...
		payCompleteFrontUrlAll = payCompleteFrontUrl + "?" + sb.toString();
		
		logger.info("payCompleteFrontUrlAll=" + payCompleteFrontUrlAll);
		logger.info("==========================exceptionOpe end==========================");
		return payCompleteFrontUrlAll;
	}catch(BizException bizE){
		logger.error(bizE.getMessage(), bizE);
		throw bizE;
	}catch(Exception e){
...
<script type="text/javascript">
    function onSubmit(){
    	...
    	//logger.info("serviceUrl=" + '<%= serverUrl%>');
