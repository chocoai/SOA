...
public class CertPay implements ITradeCommand{
	private static Logger logger = Logger.getLogger(CertPay.class);
	private final static String payInterfaceNo = Constant.PAY_INTERFACE_CERT_PAY;
	...
	public Map<String, Object> undoCommand(Map<String, Object> commandInfo)
		...
		trade_param.put("pay_channelNo", 		Constant.PAY_CHANNEL_CERT_PAY );
		...
		try{
			refundResponse = Extension.gatewayService.gatewayRefund(version, commandInfo.get("command_no").toString(), commandInfo.get("trade_money").toString(), orderDatetime, sysid, Constant.PAY_INTERFACE_CERT_PAY);
		}
		catch(Exception e){
